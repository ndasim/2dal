	...
	Shape.prototype = {
		update: function () {
			this.pos.y - opts.radius - opts.thickness < h
				? ((this.pos.y += opts.gravity), (this.angle += opts.rotSpeed * this.d))
		...
		render: function () {
			if (this.vertices > 0) {
				$.beginPath();
				$.moveTo(
					Math.cos(this.angle * piDeg) * opts.radius + this.pos.x,
					Math.sin(this.angle * piDeg) * opts.radius + this.pos.y
				);
